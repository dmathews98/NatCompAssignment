...
class PSO: # all the material that is relavant at swarm leveel
    ...
    def __init__(self, w, a1, a2, a3, dim, population_size, time_steps, search_range, fitness_func, constrainer, verbose=True):
        ...
        self.a1_percent = self.a1 / self.a_total
        self.verbose = verbose
        def as_over_time(t):
            # Slowly make attraction to global best stronger
            # Uncomment these lines to try a changing percentage of a1 over time
            #mean = self.a1_percent * (time_steps - t) / time_steps
            #val = mean * self.a_total
            #return (val, self.a_total - val)
            return (self.a1, self.a2)
        def w_over_time(t, period=10):
            # Uncomment to increase and decrease w sinusoidally with some period
            return self.w# * (np.sin(np.pi * t / period) / 2 + 1)
        self.as_over_time = as_over_time
        self.w_over_time = w_over_time
    ...
    def run(self):
        for t in range(self.time_steps):
            ...
            if ((t  + 1) % int(math.ceil(self.time_steps / 20)) == 0 or t == self.time_steps - 1) and self.verbose:
